...
function generate()
{
    ...
    global $hsize;  // horizontal length of box
    global $vsize;  // vertical height of box
    global $chosengen; // number of generations to display
...
step 9:   large rectangles with name, birth and death details + popup with further details');
        ...
        if ($humo_option["url_rewrite"] == 'j') {
            $path = 'descendant/' . $tree_id . '/' . $keepfamily_id . '?';
            $path2 = 'descendant/' . $tree_id . '/' . $keepfamily_id . '?';
        } else {
            $path = 'index.php?page=descendant.php&amp;tree_id=' . $tree_id . '&amp;id=' . $keepfamily_id . '&amp;';
            // Don't use &amp; for javascript.
            $path2 = 'index.php?page=descendant.php&amp;tree_id=' . $tree_id . '&id=' . $keepfamily_id . '&';
        }

        //echo '<form method="POST" name="desc_form" action="descendant/' . $tree_id . '/' . $keepfamily_id . '?chosensize=' . $size . '" style="display : inline;">';
?>
