...
function generate()
{
    ...
    if ($direction == 0) { // if vertical
        ...
        if ($size == 50) {   // full size box with name and details
            $hsize = 150;
            if ($hourglass === true) $hsize = 170;
            $vsize = 75;
            $hdist = 60;
            if ($hourglass === true) $hdist = 30;
        } elseif ($size == 45) { // smaller box with name + popup
            $hsize = 100;
            $vsize = 45;
            $hdist = 50;
            ...
            $vsize = $size;
            $hdist = $size;
            if ($size < 15) $hdist = 15; // shorter than this doesn't look good
        }
...
function printchart()
{
    ...
    global $descendant_header, $humo_option;
...
step 9:   large rectangles with name, birth and death details + popup with further details');
        ...
        if ($dna == "none") {
            ...
            echo '<h1 class="standard_header fonts" style="align:center; text-align: center;"><b>' . __('Same Y-DNA as ') . $base_person_name . '</b>';
        } elseif ($dna == "mtdna" or $dna == "mtdnamark") {
            echo '<h1 class="standard_header fonts" style="align:center; text-align: center;"><b>' . __('Same mtDNA as ') . $base_person_name . '</b>';
        }
        ...
        if ($humo_option["url_rewrite"] == 'j') {
            $path = CMS_ROOTPATH . 'descendant/' . $tree_id . '/' . $keepfamily_id . '?';
            $path2 = CMS_ROOTPATH . 'descendant/' . $tree_id . '/' . $keepfamily_id . '?';
        } else {
            //$path = CMS_ROOTPATH . 'descendant.php?tree_id=' . $tree_id . '&amp;id=' . $keepfamily_id . '&amp;';
            $path = CMS_ROOTPATH . 'index.php?page=descendant.php&amp;tree_id=' . $tree_id . '&amp;id=' . $keepfamily_id . '&amp;';
            // Don't use &amp; for javascript.
            //$path2 = CMS_ROOTPATH . 'descendant.php?tree_id=' . $tree_id . '&id=' . $keepfamily_id . '&';
            $path2 = CMS_ROOTPATH . 'index.php?page=descendant.php&amp;tree_id=' . $tree_id . '&id=' . $keepfamily_id . '&';
        }
...
?>
        <form method="POST" name="desc_form" action="<?= $path . 'chosensize=' . $size; ?>" style="display : inline;">
            <?php
    ...
    <?php
        ...
        for ($i = 2; $i <= 15; $i++) {
            ...
            echo '<option value="' . $path . 'main_person=' . $keepmain_person . '&amp;direction=' . $direction . '&amp;dnachart=' . $dna .
        ...
        echo '<option value="' . $path . 'main_person=' . $keepmain_person . '&amp;direction=' . $direction . '&amp;database=' . $database .
        ...
        echo '
            ...
            $(function() {
                ...
                $("#slider").on("slidestop", function(event, ui) {
                    endPos = ui.value;
                    if (startPos != endPos) {
                        window.location.href = "' . $path2 . 'main_person=' . $keepmain_person .
