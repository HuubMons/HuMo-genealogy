...
class TreeCheckChangesModel
{
    ...
    public function get_show_persons()
    {
        $show_persons = false;
    ...
    public function get_show_families()
    {
        $show_families = false;
    ...
    public function get_changes($dbh, $db_functions, $tree_check, $tree_id)
    {
        ...
        if ($tree_check['show_persons']) {
            if ($tree_check['editor']) {
                ...
                $person_qry = "(SELECT *, pers_changed_datetime AS changed_datetime
                    FROM humo_persons WHERE pers_tree_id='" . $tree_id . "' AND pers_changed_datetime IS NOT NULL AND pers_changed_user_id='" . $tree_check['editor'] . "')
                    UNION (SELECT *, pers_new_datetime AS changed_datetime
                    FROM humo_persons WHERE pers_tree_id='" . $tree_id . "' AND pers_changed_datetime IS NULL AND pers_new_user_id='" . $tree_check['editor'] . "')
        ...
        if ($tree_check['show_families']) {
            if ($tree_check['editor']) {
                ...
                $person_qry = "(SELECT *, fam_changed_datetime AS changed_datetime
                    FROM humo_families WHERE fam_tree_id='" . $tree_id . "' AND fam_changed_datetime IS NOT NULL AND fam_changed_user_id='" . $tree_check['editor'] . "')
                    UNION (SELECT *, fam_new_datetime AS changed_datetime
                    FROM humo_families WHERE fam_tree_id='" . $tree_id . "' AND fam_changed_datetime IS NULL AND fam_new_user_id='" . $tree_check['editor'] . "')
                    ORDER BY changed_datetime DESC LIMIT 0," . $tree_check['limit'];
            } else {
                // *** Show latest changes and additions ***
                // *** Remark: ordering is done in the array, but also needed here to get good results if $tree_check['limit'] is a low value ***
    ...
    public function get_editors($dbh, $tree_id, $tree_check)
    {
        ...
        return $dbh->query($select_editor_qry);
    }
}
