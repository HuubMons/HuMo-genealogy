<?php
class SettingsHomepageModel
{
    public function reset_modules($dbh)
    {
        // *** Reset all modules. TODO: add confirmation box? ***
        if (isset($_GET['template_homepage_reset']) && $_GET['template_homepage_reset'] == '1') {
            $dbh->query("DELETE FROM humo_settings WHERE setting_variable='template_homepage'");
    ...
    public function save_settings_modules($dbh)
    {
        // *** Change Module ***
    ...
    public function save_settings_favorites($dbh)
    {
        ...
        if (isset($_GET['up']) && is_numeric($_GET['link_order']) && is_numeric($_GET['id'])) {
            ...
            $item = $dbh->query("SELECT * FROM humo_settings WHERE setting_variable='link' AND setting_order=" . ($_GET['link_order'] - 1));
