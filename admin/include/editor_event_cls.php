...
class editor_event_cls
{
	...
	function hide_show_start($data_listDb, $alternative_text = '')
	{
		...
		if ($event_event or $data_listDb->event_text) {
			...
			if ($data_listDb->event_text) $text .= ' <img src="/theme/images/text.png" height="16" alt="' . __('text') . '">';
			$text .= '</span><br>';
		}
	...
	*/ elseif ($event_kind == 'picture') {
		...
		if ($event_kind == 'person') {
			...
			$text .= '<tr class="table_header_large" id="event_person_link"><td style="border-right:0px;">' . __('Events') . '</td>';
		...
		if ($event_kind == 'family') {
			...
			$text .= '<tr class="table_header_large" id="event_family_link"><td style="border-right:0px;">' . __('Events') . '</td>';
			$text .= '<td style="border-right:0px;">';
			//$text.='<a href="index.php?'.$joomlastring.'page='.$page.'&amp;menu_admin=person&amp;event_add=add_event">'.__('Add').'</a>';
			$text .= '</td><td style="border-left:0px;">';
		...
		if ($event_kind == 'birth_declaration') {
			...
			$text .= '<td style="border-left:0px;">';
			//$text.='<a href="index.php?' . $joomlastring . 'page=' . $page . '&amp;menu_admin=person&amp;event_add=add_birth_declaration">['.__('Add').']</a>';
			$text .= ' <input type="Submit" name="add_birth_declaration" value="' . __('Add') . '">';
			$text .= '</td>';
			$text .= '<td></td>';
			$text .= '</tr>';
		}
		...
		if ($event_kind == 'baptism_witness') {
			...
			$text .= '<td style="border-left:0px;">';
			//$text .= '<a href="index.php?' . $joomlastring . 'page=' . $page . '&amp;menu_admin=person&amp;event_add=add_baptism_witness">['.__('Add').']</a>';
			$text .= ' <input type="Submit" name="add_baptism_witness" value="' . __('Add') . '">';
			$text .= '</td>';
			$text .= '<td></td>';
			$text .= '</tr>';
		}
		...
		if ($event_kind == 'death_declaration') {
			...
			$text .= '<td style="border-left:0px;">';
			//$text.='<a href="index.php?'.$joomlastring.'page='.$page.'&amp;menu_admin=person&amp;event_add=add_death_declaration">['.__('Add').']</a>';
			$text .= ' <input type="Submit" name="add_death_declaration" value="' . __('Add') . '">';
			$text .= '</td>';
			$text .= '<td></td>';
			$text .= '</tr>';
		}
		...
		if ($event_kind == 'burial_witness') {
			...
			$text .= '<td style="border-left:0px;">';
			//$text.='<a href="index.php?'.$joomlastring.'page='.$page.'&amp;menu_admin=person&amp;event_add=add_burial_witness">['.__('Add').']</a>';
			$text .= ' <input type="Submit" name="add_burial_witness" value="' . __('Add') . '">';
			$text .= '</td>';
			$text .= '<td></td>';
			$text .= '</tr>';
		}
		...
		if ($event_kind == 'profession') {
			$text .= '<tr class="table_header_large" id="profession">';
			$text .= '<td style="border-right:0px;">';
			$link = 'profession';
		...
		if ($event_kind == 'religion') {
			$text .= '<tr class="table_header_large" id="religion">';
			$text .= '<td style="border-right:0px;">';
			$link = 'religion';
			$text .= __('Religion') . '</td>';
			$text .= '<td style="border-right:0px;"></td>';
			...
			*/
			...
			while($data_listDb=$data_list_qry->fetch(PDO::FETCH_OBJ)){
				...
				if ($count>1) {
					...
					$show_image= '<img src="/theme/admin/images/drag-icon.gif" style="float:left;vertical-align:top;height:16px;">'; $text.=$show_image;
					$text.='</div>';
				}
				...
				if($extensions_check=="jpg" OR $extensions_check=="gif" OR $extensions_check=="png" OR $extensions_check=="bmp") {
					if (file_exists($path_prefix.$tree_pict_path2.$thumb_prefix.$data_listDb->event_event))
						...
						$show_image= '<img src="/theme/images/thumb_missing-image.jpg" height="60px">';
					if (!$data_listDb->event_event) $show_image= '&nbsp;<img src="/theme/images/thumb_missing-image.jpg" height="60px">';
					$text.=$show_image;
				}
				else {
					$ext = substr($data_listDb->event_event,-3,3);
					if($ext=="tif" OR $ext=="iff") { $text.='<span style="font-size:80%">['.__('Format not supported')."]</span>"; }
					elseif($ext=="pdf") { $text.='<img src="/theme/images/pdf.jpeg" style="width:30px;height:30px;">';}
					elseif($ext=="doc" OR $ext=="ocx") { $text.='<img src="/theme/images/msdoc.gif" style="width:30px;height:30px;">';}
					elseif($ext=="avi" OR $ext=="wmv" OR $ext=="mpg" OR $ext=="mp4" OR $ext=="mov") { $text.='<img src="/theme/images/video-file.png" style="width:30px;height:30px;">'; }
					elseif($ext=="wma" OR $ext=="wav" OR $ext=="mp3" OR $ext=="mid" OR $ext=="ram" OR $ext==".ra" ) { $text.='<img src="/theme/images/audio.gif" style="width:30px;height:30px;">';}

					$text.='<br><span style="font-size:85%">'.$data_listDb->event_event.'</span>';
				
				}
				// *** No picture selected yet, show dummy picture ***
				if (!$data_listDb->event_event) $text.='<img src="/theme/images/thumb_missing-image.jpg" height="60px">';
				$text.='</div>';
				$text.='</div>';
				$text.='</li>';
			} 
			...
			if ($event_connect_kind == 'person' or $event_connect_kind == 'family') {
				...
				while ($connectDb = $connect_sql->fetch(PDO::FETCH_OBJ)) {
					...
					while ($pictureDb = $picture_qry->fetch(PDO::FETCH_OBJ)) {
						...
						//event_connect_kind and event_connect_id
						//echo $pictureDb->event_connect_kind.'TEST';
						//if ($pictureDb->event_connect_kind OR $pictureDb->event_connect_id){
						//	echo 'Object is used multiple times';
		...
		if (!isset($_GET['add_person'])) {
			...
			while ($data_listDb = $data_list_qry->fetch(PDO::FETCH_OBJ)) {
				...
				$text .= '"><img src="/theme/admin/images/button_drop.png" border="0" alt="down"></a>';
				...
				if ($data_listDb->event_order < $count) {
					...
					// *** Edit picture by source in seperate source page ***
					if ($event_kind == 'source_picture') $text .= '&amp;source_id=' . $data_listDb->event_connect_id;
					$text .= '&amp;dummy=' . $data_listDb->event_id . $internal_link . '"><img src="/theme/admin/images/arrow_down.gif" border="0" alt="down"></a>';
				} else {
					$text .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				}
				...
				if ($data_listDb->event_kind == 'picture') {
					...
					if (strtolower($extensions_check) == "pdf") {
						$text .= '<a href="' . $path_prefix . $tree_pict_path3 . $data_listDb->event_event . '"><img src="/theme/images/pdf.jpeg"></a>';
					} elseif (strtolower($extensions_check) == "doc" or strtolower(substr($path_prefix . $tree_pict_path3 . $data_listDb->event_event, -4, 4)) == "docx") {
						$text .= '<a href="' . $path_prefix . $tree_pict_path3 . $data_listDb->event_event . '"><img src="/theme/images/msdoc.gif"></a>';
					}
					// *** Show AVI Video file ***
					elseif ($extensions_check == "avi") {
						$text .= '<a href="' . $path_prefix . $tree_pict_path3 . $data_listDb->event_event . '" target="_blank"><img src="/theme/images/video-file.png"></a>';
					}
					// *** Show WMV Video file ***
					...
					elseif (strtolower($extensions_check) == "wma") {
						$text .= '<a href="' . $path_prefix . $tree_pict_path3 . $data_listDb->event_event . '" target="_blank"><img src="/theme/images/audio.gif"></a>';
					}
					// *** Show WAV Audio file ***
					...
					elseif (strtolower($extensions_check) == ".ra") {
						...
						if ($data_listDb->event_event and file_exists($picture)) {
							...
							$show_image = '<img src="/theme/images/thumb_missing-image.jpg" style="width:100px">';
						//Check line above. If thumb if missing, missing picture is shown...

						if (!$data_listDb->event_event) $show_image = '<img src="/theme/images/thumb_missing-image.jpg" style="width:100px">';
						$text .= $show_image;
					}
				}
				...
				if (
					...
					//$text.='<a href="javascript:;" onClick=window.open("index.php?page=editor_media_select&amp;form='.$form.'&amp;event_id='.$data_listDb->event_id.'","","width=400,height=500,top=100,left=100,scrollbars=yes");><img src="/theme/images/search.png" border="0"></a>';
					$text .= '<a href="javascript:;" onClick=window.open("index.php?page=editor_media_select&amp;form=' . $form . '&amp;event_id=' . $data_listDb->event_id . '","","' . $field_popup . '");><img src="/theme/images/search.png" border="0"></a>';
					...
					$text .= '<a href="javascript:;" onClick=window.open("index.php?page=editor_relation_select&amp;adoption_id=' . $data_listDb->event_id . '","","' . $field_popup . '");><img src="/theme/images/search.png" border="0"></a>';
				}
				...
				if ($event_connect_kind == 'family') $form = 2;
				if ($event_connect_kind == 'source') $form = 3;
				//$text.='<a href="javascript:;" onClick=window.open("index.php?page=editor_place_select&amp;form='.$form.'&amp;place_item=event_place&amp;event_id='.$data_listDb->event_id.'","","width=400,height=500,top=100,left=100,scrollbars=yes");><img src="/theme/images/search.png" border="0"></a>';
				$text .= '<a href="javascript:;" onClick=window.open("index.php?page=editor_place_select&amp;form=' . $form . '&amp;place_item=event_place&amp;event_id=' . $data_listDb->event_id . '","","' . $field_popup . '");><img src="/theme/images/search.png" border="0"></a>';
