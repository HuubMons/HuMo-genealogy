...
if (!isset($groupDb->group_googlemaps)) {
    $user['group_googlemaps'] = 'j';
} else {
    $user['group_googlemaps'] = $groupDb->group_googlemaps;
}

if (!isset($groupDb->group_contact)) {
    $user['group_contact'] = 'j';
} else {
    $user['group_contact'] = $groupDb->group_contact;
}

if (!isset($groupDb->group_latestchanges)) {
    $user['group_latestchanges'] = 'j';
} else {
    $user['group_latestchanges'] = $groupDb->group_latestchanges;
}

if (!isset($groupDb->group_menu_cms)) {
    $user['group_menu_cms'] = 'y';
} else {
    $user['group_menu_cms'] = $groupDb->group_menu_cms;
}

if (!isset($groupDb->group_menu_persons)) {
    $user['group_menu_persons'] = 'j';
} else {
    $user['group_menu_persons'] = $groupDb->group_menu_persons;
}

if (!isset($groupDb->group_menu_names)) {
    $user['group_menu_names'] = 'j';
} else {
    $user['group_menu_names'] = $groupDb->group_menu_names;
}

$user['group_menu_places'] = $groupDb->group_menu_places;

if (!isset($groupDb->group_menu_login)) {
    $user['group_menu_login'] = 'j';
} else {
    $user['group_menu_login'] = $groupDb->group_menu_login;
}
...
if (!isset($groupDb->group_photobook)) {
    $user['group_photobook'] = 'n';
} else {
    $user['group_photobook'] = $groupDb->group_photobook;
}
...
if (!isset($groupDb->group_pdf_button)) {
    $user['group_pdf_button'] = 'y';
} else {
    $user['group_pdf_button'] = $groupDb->group_pdf_button;
}

if (!isset($groupDb->group_rtf_button)) {
    $user['group_rtf_button'] = 'n';
} else {
    $user['group_rtf_button'] = $groupDb->group_rtf_button;
}
...
if (!isset($groupDb->group_pers_hide_totally)) {
    $user['group_pers_hide_totally'] = 'X';
} else {
    $user['group_pers_hide_totally'] = $groupDb->group_pers_hide_totally;
}

if (!isset($groupDb->group_filter_date)) {
    $user['group_filter_date'] = 'n';
} else {
    $user['group_filter_date'] = $groupDb->group_filter_date;
}

if (!isset($groupDb->group_gen_protection)) {
    $user['group_gen_protection'] = 'n';
} else {
    $user['group_gen_protection'] = $groupDb->group_gen_protection;
}

// *** Show or hide family trees, saved as ; separated id numbers ***
if (!isset($groupDb->group_hide_trees)) {
    $user['group_hide_trees'] = '';
} else {
    $user['group_hide_trees'] = $groupDb->group_hide_trees;
}
...
if (!isset($groupDb->group_hide_photocat)) {
    $user['group_hide_photocat'] = '';
} else {
    $user['group_hide_photocat'] = $groupDb->group_hide_photocat;
}

// *** Edit family trees [GROUP SETTING], saved as ; separated id numbers (NOT USED FOR ADMINISTRATOR) ***
if (!isset($groupDb->group_edit_trees)) {
    $user['group_edit_trees'] = '';
} else {
    $user['group_edit_trees'] = $groupDb->group_edit_trees;
}
