...
if ($menu_tab == 'stats_persons') {
	...
	$percent_of_men = $both ?? ($countman / $both) * 100;
	echo '<td align="center">' . round($percent_of_men, 1) . '%</td>';
	echo "<td align='center'><i>$countwoman</i></td>\n";
	$percent_of_women = $both ?? ($countwoman / $both) * 100;
	echo '<td align="center">' . round($percent_of_women, 1) . '%</td>';
...
if ($menu_tab == 'stats_surnames') {
	...
	function last_names($max)
	{
		...
		$sql = "SELECT pers_lastname, pers_prefix, COUNT(pers_lastname) AS count_last_names
				FROM humo_persons
		...
		$row = $freq_last_names ? round(count($freq_last_names) / $maxcols) : 0;
		...
		return $freq_count_last_names[0] ?? 0; // TODO: need testing
	}
